...
int main(int argc, char **argv)
{
    ...
    float deg_arg = atof(argv[1]);
    printf("(%s) rotate %.2f degrees CW\n", TESTENV, deg_arg);
    ...
    #else
    printf("(%s) WARNING : undefined reference to MYRIO\n", TESTENV);
    #endif

    while(ros::ok){
      dataInit();

      if(timer_ticks <= (int) (deg_arg * ROTATE_CONST)) {
        TURN_RIGHT      
      } else {
        printf("(%s) finished. rotated %.3f degrees(%d timer ticks)\n", TESTENV, deg_arg, timer_ticks);
        break;
      }
